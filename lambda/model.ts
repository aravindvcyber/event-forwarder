...
export const toDataModel = (
  event: CloudFormationStackEventBridgeEvents
): CloudformationEventDbModel => {
  ...
  const clientRequestToken: { S: string } = {
    S: event["detail"]["client-request-token"] || "",
  };
  ...
  const account: { S: string } = { S: event.account };
  if (type.S === CloudFormationStackEventBridgeEvent.Resource_Change) {
    ...
    type.S === CloudFormationStackEventBridgeEvent.Drift_Detection_Change
  ) {
...
export function timeOrder(
  a: CloudformationEventDbModel,
  b: CloudformationEventDbModel
) {
